...
function buildConclusionContext(state, ticker) {
  ...
  return {
    bias: safeState.bias,
    biasSkipped: safeState.biasSkipped,
    biasConclusion,
    orderflow,
    setup,
    progress,
    ...
    orderflowEnabled: safeState.premarket.orderflowEnabled !== false,
    htfKeyLevel: safeState.premarket.htfKeyLevel,
    priceAtLevel: safeState.premarket.priceAtLevel,
    liquidityState: safeState.premarket.liquidityState,
    rangeState: safeState.premarket.rangeState,
...
export const CONCLUSION_RULES = [
  ...
  },
  {
    ...
    when: (ctx) =>
      ctx.orderflowEnabled && ctx.cvdState === "Diverging from price",
  ...
  {
    id: "orderflow-disabled",
    when: (ctx) => !ctx.orderflowEnabled,
    explain: "Orderflow inputs are turned off",
    verdict: "PROFILE / LEVEL READ",
    tone: "muted",
    title: "Key levels only",
    detail:
      "Orderflow is disabled, so the conclusion leans on the key level in play and price action at it. Re-enable orderflow for the full read.",
    reasons: (ctx) => [
      ctx.htfKeyLevel ? `Key level: ${ctx.htfKeyLevel}` : "No key level selected",
      ctx.priceAtLevel ? `Price action: ${ctx.priceAtLevel}` : "No price action selected"
    ]
  },
  {
    id: "sweep-reversal",
    when: (ctx) =>
      ctx.orderflowEnabled &&
      ctx.priceAtLevel === "Tapped / swept the level (liquidity taken)" &&
      (ctx.cvdState === "Aggression absorbed" ||
        ctx.cvdState === "Diverging from price" ||
        ctx.tapeState === "Exhaustion into level"),
    explain: "Price swept the level and the move was absorbed / exhausted",
    verdict: "SWEEP → FADE",
    tone: "warn",
    title: "Liquidity tapped, move exhausted",
    detail:
      "Price took the stops at the level but aggression got absorbed. Fade the sweep back into value on confirmation — do not chase the breakout.",
    reasons: (ctx) => [
      `Level tapped: ${ctx.htfKeyLevel}`,
      `CVD / tape: ${ctx.cvdState} / ${ctx.tapeState}`
    ]
  },
  {
    ...
    when: (ctx) =>
      ctx.orderflowEnabled &&
      ctx.priceAtLevel === "Tapped / swept the level (liquidity taken)" &&
      ctx.cvdState === "Aligned with price" &&
      (ctx.tapeState === "Fast tape with size" || ctx.tapeState === "Big prints defending level"),
    explain: "Price swept the level with aligned aggression",
    verdict: "SWEEP → CONTINUATION",
    tone: "bull",
    title: "Liquidity taken with real aggression",
    detail:
      "The level was swept and the buying / selling was real (aligned CVD + tape). Favor continuation in the direction of the sweep with a stop under the sweep.",
    reasons: (ctx) => [
      `Level swept: ${ctx.htfKeyLevel}`,
      "CVD aligned with price",
      ctx.tapeState
    ]
  },
  {
    id: "fvg-reaction",
    when: (ctx) =>
      ctx.htfKeyLevel === "HTF fair value gap" &&
      (ctx.priceAtLevel === "Rejected from the level" ||
        ctx.priceAtLevel === "Holding / consolidating at the level"),
    explain: "Price is interacting with an HTF fair value gap",
    verdict: "FVG REACTION",
    tone: "warn",
    title: "Watch the imbalance",
    detail:
      "Price is at an HTF FVG. If it holds the gap edge, the imbalance often fills — if it closes the gap fully, expect continuation through. React to the close / hold, not the touch.",
    reasons: (ctx) => [
      `Key level: ${ctx.htfKeyLevel}`,
      `Price action: ${ctx.priceAtLevel}`
    ]
  },
  {
    id: "vah-val-fade",
    when: (ctx) =>
      ctx.htfKeyLevel === "Value area edge (VAH / VAL)" &&
      ctx.rangeState === "At the range extreme (VAH / VAL)" &&
      (ctx.cvdState === "Aggression absorbed" || ctx.tapeState === "Exhaustion into level"),
    explain: "Price reached the value area edge with exhaustion",
    verdict: "VAH / VAL FADE",
    tone: "warn",
    title: "Mean reversion candidate",
    detail:
      "Price tagged VAH or VAL at the range edge and the move shows exhaustion. Fade the extreme back toward the POC / mid-value with confirmation — manage it as a mean-reversion trade.",
    reasons: (ctx) => [
      `Level: ${ctx.htfKeyLevel}`,
      ctx.cvdState,
      ctx.tapeState
    ]
  },
  {
    id: "range-fade",
    when: (ctx) =>
      ctx.rangeState === "In balance (range bound)" &&
      (ctx.htfKeyLevel === "Value area edge (VAH / VAL)" ||
        ctx.htfKeyLevel === "Open / VWAP reference"),
    explain: "Price is range bound near a reference level",
    verdict: "RANGE — FADE EXTREMES",
    tone: "warn",
    title: "Stay inside the range",
    detail:
      "The market is in balance. Fade the range extremes (VAH / VAL, range high / low) and avoid chasing the middle. Stop beyond the range edge.",
    reasons: (ctx) => [
      "Range bound",
      `Reference: ${ctx.htfKeyLevel}`
    ]
  },
  {
    id: "range-breakout",
    when: (ctx) =>
      (ctx.rangeState === "Breaking the range high" ||
        ctx.rangeState === "Breaking the range low") &&
      ctx.orderflowEnabled &&
      ctx.cvdState === "Aligned with price" &&
      (ctx.tapeState === "Fast tape with size" || ctx.tapeState === "Big prints defending level"),
    explain: "Range break with aligned orderflow",
    verdict: "RANGE BREAK → CONTINUATION",
    tone: "bull",
    title: "Trade the expansion",
    detail:
      "Price broke the range with genuine participation. Favor continuation on the break and a retest hold — add on the retest, not on the first spike.",
    reasons: (ctx) => [
      ctx.rangeState,
      "CVD aligned with price",
      ctx.tapeState
  ...
  {
    id: "level-tapped-wait",
    when: (ctx) =>
      ctx.priceAtLevel === "Tapped / swept the level (liquidity taken)" &&
      ctx.liquidityState === "No clean liquidity yet",
    explain: "Price tapped a level but there is no clean liquidity set yet",
    verdict: "WAIT FOR THE POOL",
    tone: "muted",
    title: "No liquidity setup yet",
    detail:
      "The level was tapped but there is no clear liquidity pool above or below yet. Stand aside until a clean pool forms — do not trade a touch in no-man's-land.",
    reasons: (ctx) => [
      `Level tapped: ${ctx.htfKeyLevel}`,
      "No clean liquidity yet"
    ]
  },
  {
    id: "liquidity-draw-above",
    when: (ctx) =>
      ctx.liquidityState === "Liquidity resting above" &&
      (ctx.initiative === "Buyers in control" || ctx.priceAtLevel === "Holding / consolidating at the level"),
    explain: "Liquidity is resting above and initiative favors buyers",
    verdict: "DRAW ON ABOVE",
    tone: "bull",
    title: "Target the buy-side pool",
    detail:
      "Liquidity rests above and buyers are in control. Look to trade the continuation / reclaim into the resting pool above, with your stop under the recent swing.",
    reasons: (ctx) => [
  ...
  {
    id: "liquidity-draw-below",
    when: (ctx) =>
      ctx.liquidityState === "Liquidity resting below" &&
      (ctx.initiative === "Sellers in control" || ctx.priceAtLevel === "Rejected from the level"),
    explain: "Liquidity is resting below and initiative favors sellers",
    verdict: "DRAW ON BELOW",
    tone: "bear",
    title: "Target the sell-side pool",
    detail:
      "Liquidity rests below and sellers are in control. Look to trade the continuation into the resting pool below, with your stop above the recent swing.",
    reasons: (ctx) => [
...
export function buildFinalConclusion(state, ticker) {
  ...
  if (
    ...
    warnings.push("Level was tapped without a confirmed liquidity pool");
  }
  if (
    ctx.orderflowEnabled &&
    (ctx.rangeState === "Breaking the range high" || ctx.rangeState === "Breaking the range low") &&
    ctx.cvdState &&
    ctx.cvdState !== "Aligned with price"
  ) {
    warnings.push("Range break lacks aligned CVD confirmation");
  }
  if (
    ctx.rangeState === "In balance (range bound)" &&
    ctx.priceAtLevel === "Mid-range / nowhere"
  ) {
    warnings.push("Price is in the middle of balance; wait for an edge");
  }
  ...
  const requiredSignals = [
    ...
    ctx.liquidityState,
    ctx.rangeState,
    ctx.bias || ctx.biasSkipped,
    !ctx.orderflowEnabled || (ctx.orderFlow && ctx.cvdState && ctx.tapeState)
  ];
  const signalCoverage = requiredSignals.filter(Boolean).length / requiredSignals.length;
  const confidence =
    ctx.progress.completedStages < ctx.progress.totalStages
      ? "low"
      : warnings.length >= 2 || signalCoverage < 0.65
        ? "low"
        : warnings.length === 1 || signalCoverage < 0.9
          ? "medium"
          : "high";

  const guidanceByRule = {
    "no-trade-veto": {
      action: "Stand aside and keep collecting context.",
      confirmation: "Only resume after the execution lane is changed and a valid setup appears.",
      invalidation: "Any forced entry violates the selected no-trade condition."
    },
    "cvd-divergence-veto": {
      action: "Wait. Do not chase the current move.",
      confirmation: "CVD must realign with price or a clear reclaim/rejection must form.",
      invalidation: "Price continues without participation or divergence expands."
    },
    "sweep-reversal": {
      action: "Wait for the sweep to fail, then consider the fade back into value.",
      confirmation: "Rejection, reclaim, or structure shift away from the swept level.",
      invalidation: "Acceptance beyond the level with renewed aggressive flow."
    },
    "sweep-continuation": {
      action: "Favor continuation after the sweep and controlled retest.",
      confirmation: "Aligned CVD, defending tape, and a retest that holds the level.",
      invalidation: "Price returns through the swept level and CVD loses alignment."
    },
    "fvg-reaction": {
      action: "Wait for the FVG edge to show a real hold or rejection.",
      confirmation: "Close away from the gap with a clean response at the edge.",
      invalidation: "The FVG is fully closed and price accepts through it."
    },
    "vah-val-fade": {
      action: "Look for a measured fade from VAH/VAL toward POC.",
      confirmation: "Exhaustion or absorption at the value edge, then rejection.",
      invalidation: "Acceptance outside value with initiative follow-through."
    },
    "range-fade": {
      action: "Trade only the range edges; avoid the middle.",
      confirmation: "Rejection at VAH/VAL or the range high/low.",
      invalidation: "A confirmed range break and acceptance outside balance."
    },
    "range-breakout": {
      action: "Favor expansion after a confirmed break and retest.",
      confirmation: "Aligned CVD, supportive tape, and a retest that holds.",
      invalidation: "Breakout failure back inside the prior range."
    },
    "orderflow-disabled": {
      action: "Use only the selected key level, price action, and liquidity map.",
      confirmation: "Re-enable orderflow before taking a high-conviction entry.",
      invalidation: "A level-only read is not enough for a full execution decision."
    }
  };
  const guidance = guidanceByRule[rule.id] || {
    action: rule.detail,
    confirmation: "Wait for the selected scenario and profile to agree before entry.",
    invalidation: "The selected level, narrative, or orderflow condition fails."
  };
  ...
  return {
    ...
    signals: [
      ...
      { key: "Bias", value: ctx.biasSkipped ? "Skipped" : formatBiasLabel(ctx.bias) },
      { key: "Profile map", value: ctx.setup.label },
      {
        key: "Orderflow",
        value: ctx.orderflowEnabled ? ctx.orderflow.label : "Off"
      },
      {
        key: "Key level",
        value: ctx.htfKeyLevel || "-"
      },
      {
        key: "Price at level",
        value: ctx.priceAtLevel || "-"
      },
...
export function getProtocolProgress(state, ticker) {
  ...
  const dailyPrepComplete = Boolean(
    (safeState.bias || safeState.biasSkipped) &&
  ...
  const orderflowEnabled = safeState.premarket.orderflowEnabled !== false;
  const orderflowCore =
    safeState.premarket.htfKeyLevel &&
    safeState.premarket.priceAtLevel &&
    safeState.premarket.liquidityState;
...
export function getGoNoGo(state, ticker) {
  ...
  if (!safeState.bias && !safeState.biasSkipped) blockers.push("Bias not locked");
  ...
  if (!safeState.premarket.htfKeyLevel) blockers.push("HTF key level in play missing");
  if (!safeState.premarket.priceAtLevel) blockers.push("Price action at level missing");
  if (!safeState.premarket.liquidityState) blockers.push("Liquidity state missing");
  ...
  if (orderflowEnabled && safeState.premarket.cvdState === "Diverging from price") {
  ...
  if (safeState.premarket.rangeState === "In balance (range)" && safeState.premarket.priceAtLevel === "Mid-range / nowhere") {
    blockers.push("Price is mid-range with no edge");
  }
...
export function calculateMetrics(state, ticker) {
  ...
  const orderflowEnabled = safeState.premarket.orderflowEnabled !== false;
  const progressItems = [
    Boolean(safeState.bias || safeState.biasSkipped),
    safeState.dailyPrep.waitFirstThirty,
    safeState.dailyPrep.levelsMarked,
    safeState.dailyPrep.eventRiskChecked,
    ...
    Boolean(safeState.premarket.htfKeyLevel),
    Boolean(safeState.premarket.priceAtLevel),
    Boolean(safeState.premarket.liquidityState),
    !orderflowEnabled || Boolean(safeState.premarket.htfPoi),
    !orderflowEnabled || Boolean(safeState.premarket.orderFlow),
    !orderflowEnabled || Boolean(safeState.premarket.cvdState),
    !orderflowEnabled || Boolean(safeState.premarket.tapeState),
    !orderflowEnabled || Boolean(safeState.premarket.executionLane),
    !orderflowEnabled || Boolean(safeState.premarket.initiative),
    !orderflowEnabled || Boolean(safeState.premarket.response)
  ];
...
export function createMarkdown({ ticker, dateLabel, state }) {
  ...
  const lines = [
    ...
    `**Bias:** ${safeState.biasSkipped ? "Skipped" : formatBiasLabel(safeState.bias)}`,
    ...
    `- Orderflow enabled: ${safeState.premarket.orderflowEnabled === false ? "No" : "Yes"}`,
    `- HTF key level in play: ${safeState.premarket.htfKeyLevel || "-"}`,
    `- Price action at level: ${safeState.premarket.priceAtLevel || "-"}`,
    `- Liquidity state: ${safeState.premarket.liquidityState || "-"}`,
    `- Range / balance state: ${safeState.premarket.rangeState || "-"}`,
