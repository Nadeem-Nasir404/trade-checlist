...
function buildBlocks({ ticker, day, state }) {
  ...
  const blocks = [
    ...
    paragraphBlock(`Orderflow enabled: ${safeState.premarket.orderflowEnabled === false ? "No" : "Yes"}`),
    paragraphBlock(`HTF key level in play: ${safeState.premarket.htfKeyLevel || "-"}`),
    paragraphBlock(`Price action at level: ${safeState.premarket.priceAtLevel || "-"}`),
    paragraphBlock(`Liquidity state: ${safeState.premarket.liquidityState || "-"}`),
    paragraphBlock(`Range / balance state: ${safeState.premarket.rangeState || "-"}`),
